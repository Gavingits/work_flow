...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        # The project/workflow directory, chosen by the user.
        self.wfpath = None
        # The directory where the application itself is running.
        ...
        self.init_ui()
        self.create_menus()
        self.discover_tools() # Discover tools on startup
    ...
    def init_ui(self):
        ...
        self.tools_list_widget.itemDoubleClicked.connect(self.add_node_from_item) # Double-click to add
    ...
    def create_menus(self):
        ...
        set_dir_action = QAction('设置工作流目录 (&D)...', self)
        set_dir_action.setStatusTip('选择一个文件夹来保存项目和输出文件')
    ...
    def execute_workflow(self):
        """解析图，进行拓扑排序，并执行工作流。"""
        if not self.wfpath:
            QMessageBox.warning(self, "错误", "请先通过“文件”->“设置工作流目录”来指定一个项目保存路径。")
            return
        ...
        adj = {node_id: [] for node_id in nodes_map}; in_degree = {node_id: 0 for node_id in nodes_map}; predecessors = {node_id: None for node_id in nodes_map}
        for conn in connections:
            start_node, end_node = conn.start_socket.parentItem(), conn.end_socket.parentItem()
            if start_node and end_node and start_node.id in adj and end_node.id in in_degree:
                adj[start_node.id].append(end_node.id); in_degree[end_node.id] += 1; predecessors[end_node.id] = start_node.id
        queue = deque([node_id for node_id in nodes_map if in_degree[node_id] == 0]); execution_order = []
        while queue:
            node_id = queue.popleft(); execution_order.append(node_id)
            for neighbor_id in adj[node_id]:
                in_degree[neighbor_id] -= 1
                if in_degree[neighbor_id] == 0: queue.append(neighbor_id)
        if len(execution_order) != len(nodes_map):
            QMessageBox.critical(self, "错误", "工作流中存在循环，无法执行。"); return
        ...
        try:
            for node_id in execution_order:
                ...
                print(f"    输入路径: {input_path}"); print(f"    输出路径: {output_path}")

                tool_module_path = os.path.join(self.app_root, 'tools', f"{node.name}.py")
    ...
    def on_save_workflow(self):
        if not self.wfpath:
            QMessageBox.warning(self, "提示", "请先设置一个工作流目录。"); return
        filepath, _ = QFileDialog.getSaveFileName(self, "保存工作流", self.wfpath, "Workflow Files (*.json)")
        if filepath: self.save_workflow(filepath)
    ...
    def on_load_workflow(self):
        ...
        if filepath: self.load_workflow(filepath)
    ...
    def save_workflow(self, filepath):
        try:
            ...
            for node in nodes:
                workflow_data['nodes'].append({'id': node.id, 'name': node.name, 'pos': [node.pos().x(), node.pos().y()], 'config': node.config})
            for conn in connections:
                workflow_data['connections'].append({'start_node_id': conn.start_socket.parentItem().id, 'end_node_id': conn.end_socket.parentItem().id})
    ...
    def load_workflow(self, filepath):
        try:
            ...
            for node_data in workflow_data.get('nodes', []):
                ...
                node.id, node.config = node_data['id'], node_data.get('config', {})
            ...
            for conn_data in workflow_data.get('connections', []):
                ...
                if start_node and end_node:
                    self.scene.addItem(Connection(start_node.socket_output, end_node.socket_input))
            QMessageBox.information(self, "成功", f"工作流已从\n{filepath}\n成功加载")
            # Set the current wfpath to the directory of the loaded file
            self.wfpath = os.path.dirname(filepath)
            self.setWindowTitle(f"工作流编辑器 - {self.wfpath}")
        except Exception as e:
